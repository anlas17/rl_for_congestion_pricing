...
class Simulation():
    ...
    def __init__(self, params):
        ...
        #for i in range(self.Number_of_user):
            #self.E[i] = 0.5+np.random.normal(0.05**2,0.1**2)
            #self.L[i] = 4+np.random.normal(0.1**2,0.4**2)
            #while self.E[i]<0.3 or self.E[i]>0.7:
                #self.E[i] = 0.5+np.random.normal(0.05**2,0.1**2)
            #while self.L[i]<2.5 or self.L[i]>5.5:
                #self.L[i] = 4+np.random.normal(0.1**2,0.4**2)
...
class CommuteEnv(Env):
    def __init__(self):
        ...
        # mu, sigma, A
        self.action_space = Box(low=np.array([-1.0, -1.0, -1.0]), high=np.array([1.0, 1.0, 1.0]), shape=(3,), dtype=np.float32)
